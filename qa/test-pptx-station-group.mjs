// qa/test-pptx-station-group.mjs — 역 그룹화 후처리(groupStationShapes) 단위 테스트.
// pptxgenjs로 STGRP| 태깅된 미니 프레젠테이션을 만들고, 후처리 결과 XML에
// 역 단위 p:grpSp가 생기는지/비태깅 도형은 그대로인지 검증한다. (tsx로 TS 모듈 로드)
import assert from "node:assert/strict";
import PptxGenJS from "pptxgenjs";
import JSZip from "jszip";
import { groupStationShapes, STATION_GROUP_PREFIX } from "../src/lib/pptx-station-group.ts";

const pptx = new PptxGenJS();
const slide = pptx.addSlide();
// 역A: 캐싱선 + 노선바 + 도트 + 라벨(회전) = 4개 요소
slide.addShape("line", { x: 1, y: 1, w: 1, h: 0.2, line: { color: "FFFFFF", width: 6 }, objectName: `${STATION_GROUP_PREFIX}sub-1|강남역|casing0` });
slide.addShape("line", { x: 1, y: 1, w: 1, h: 0.2, line: { color: "00A84D", width: 4 }, objectName: `${STATION_GROUP_PREFIX}sub-1|강남역|bar0` });
slide.addShape("ellipse", { x: 1.4, y: 1.05, w: 0.1, h: 0.1, fill: { color: "00A84D" }, objectName: `${STATION_GROUP_PREFIX}sub-1|강남역|dot` });
slide.addText("강남역", { x: 1.2, y: 1.3, w: 0.8, h: 0.25, rotate: 20, objectName: `${STATION_GROUP_PREFIX}sub-1|강남역|label` });
// 역B: 요소 2개
slide.addShape("line", { x: 3, y: 2, w: 1, h: 0.1, line: { color: "FFFFFF", width: 6 }, objectName: `${STATION_GROUP_PREFIX}sub-2|역삼역|casing0` });
slide.addShape("ellipse", { x: 3.4, y: 2, w: 0.1, h: 0.1, fill: { color: "00A84D" }, objectName: `${STATION_GROUP_PREFIX}sub-2|역삼역|dot` });
// 비태깅 도형 — 그룹화 대상 아님
...
const grouped = await groupStationShapes(buf);
