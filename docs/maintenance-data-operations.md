...
## 상세정보 보강·완료 판별·실거래가 (2026-07-30 추가)

- **정보몽땅 필드 채움**: 공간조인 성공 시 단계 외에 주소(대표지번), 시행자(조합명), 유형(재건축 등)을 채우고, 면적이 없으면 경계 폴리곤에서 계산한다.
- **국토부 속성 브리지**: 정보몽땅 사업장명 ↔ 국토부 통합/표준 속성 이름 매칭(같은 시군구·단일 후보)으로 예정세대수·구역지정일·용적률·건폐율 등을 채운다.
- **건축물대장 완료 판별**: 후반부 단계(관리처분·철거·착공·분양) 구역은 대표지번의 총괄표제부(`BldRgstHubService/getBrRecapTitleInfo`) 사용승인일을 조회해, 구역지정일(미상이면 2018년) 이후 신축 승인이면 준공으로 재판정·제외한다. 법정동코드는 NCP 리버스지오코딩(orders=legalcode)으로 얻고 `building_ledger_cache`(sqlite)에 캐시한다.
