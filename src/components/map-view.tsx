...
const OSM_SUBWAY_DATA_PATH = "/data/osm-subway.json";
...
interface RouteProjection extends LatLng {
  readonly distanceM: number;
  readonly offsetM: number;
  readonly segmentStart: LatLng;
  readonly segmentEnd: LatLng;
}

interface SubwayAxisShape {
...
interface SubwayEntranceShape extends LatLng {
  readonly ref: string;
  readonly station: SubwayStation;
  readonly color: string;
  readonly connectorFrom: LatLng;
}
...
function subwayLineColor(value: string | undefined): string {
  return /^#[0-9a-fA-F]{6}$/.test(value ?? "") ? value! : "#475569";
}

function subwayLineLabelText(lineRef: string | undefined, lineName?: string | undefined): string {
...
function subwayStationIconHtml(stationName: string, line: string, color: string): string {
...
function normalizedSubwayStationName(value: string): string {
  return value.replace(/\s+/g, "").replace(/역$/u, "");
}

function isPublicSubwayLineRef(value: string | undefined): boolean {
...
function isPublicSubwayAxis(axis: SubwayStationAxis): boolean {
...
function moveLatLngByMeters(origin: LatLng, dxM: number, dyM: number): LatLng {
  return {
    lat: origin.lat + dyM / METERS_PER_LAT,
    lng: origin.lng + dxM / metersPerLngAtLat(origin.lat),
  };
}
...
function projectPointOnRoute(point: LatLng, coordinates: readonly LatLngTuple[]): RouteProjection | null {
  let cursorM = 0;
  ...
  for (let index = 0; index < coordinates.length - 1; index += 1) {
    ...
    const candidate: RouteProjection = {
      ...
      offsetM: cursorM + segmentLengthM * projected.t,
      segmentStart: start,
      segmentEnd: end,
    };
...
function pushUniqueLatLng(points: LatLngTuple[], point: LatLngTuple) {
...
function sliceRouteCoordinates(
  ...
  for (let index = 0; index < coordinates.length - 1; index += 1) {
    ...
    if (segmentEndM >= startM && segmentStartM <= endM) {
      ...
      pushUniqueLatLng(points, interpolateLatLng(start, end, localStart));
      pushUniqueLatLng(points, interpolateLatLng(start, end, localEnd));
    }
...
function vectorFromLatLngs(start: LatLng, end: LatLng) {
  const metersPerLng = metersPerLngAtLat((start.lat + end.lat) / 2);
  const dx = (end.lng - start.lng) * metersPerLng;
  const dy = (end.lat - start.lat) * METERS_PER_LAT;
...
function displayableSubwayEntrances(
...
function dedupeSubwayStationAxes(axes: SubwayStationAxis[]): SubwayStationAxis[] {
...
function axisLatLngs(axis: SubwayStationAxis): LatLngTuple[] {
  return axis.endpoints.map(([lng, lat]) => [lat, lng]);
}

function subwayCoordToLatLng(coord: [number, number]): LatLng | null {
  const [lng, lat] = coord;
...
function nearestPointOnAxis(point: LatLng, axis: SubwayStationAxis): LatLng & { readonly distanceSquared: number } {
...
function stationAxesForStation(
...
function subwayEntranceConnector(
  entrance: SubwayMapEntrance,
...
function axisTouchesBounds(axis: SubwayStationAxis, bounds: import("leaflet").LatLngBounds): boolean {
...
function subwayGeometryLines(geometry: unknown): [number, number][][] {
  if (!geometry || typeof geometry !== "object") {
    return [];
  }
...
function representativeSubwayLines(
...
function subwayStationAxisShape(
...
function subwayAxisShapeTouchesBounds(
...
function osmSubwayLineLabelPlacement(
  geometry: unknown,
  bounds: import("leaflet").LatLngBounds,
...
function routeCoordinatesFor(
  route: SubwayRoute,
  stationById: ReadonlyMap<string, SubwayStation>
): LatLngTuple[] {
  if (route.coordinates && route.coordinates.length >= 2) {
    return route.coordinates.map(([lat, lng]) => [lat, lng]);
  }
...
function axisForStation(
  station: SubwayStation,
  routes: readonly SubwayRoute[],
  stationById: ReadonlyMap<string, SubwayStation>
): SubwayAxisShape {
  const route =
    routes.find((candidate) => candidate.stationIds.includes(station.id)) ??
    routes.find((candidate) => candidate.line === station.line);
  const coordinates = route ? routeCoordinatesFor(route, stationById) : [];
  const color = subwayLineColor(route?.lineColor ?? station.lineColor);
  const line = route?.line ?? station.line;
  const stationPoint = { lat: station.lat, lng: station.lng };
  const projection = coordinates.length >= 2 ? projectPointOnRoute(stationPoint, coordinates) : null;
  const center = projection && projection.distanceM <= 180 ? { lat: projection.lat, lng: projection.lng } : stationPoint;
  const halfLengthM = 78;

  if (projection && coordinates.length >= 2) {
    const latLngs = sliceRouteCoordinates(coordinates, projection.offsetM - halfLengthM, projection.offsetM + halfLengthM);
    if (latLngs.length >= 2) {
      return { station, line, color, latLngs };
    }
  }

  const direction = projection ? vectorFromLatLngs(projection.segmentStart, projection.segmentEnd) : { ux: 1, uy: 0 };
  const start = moveLatLngByMeters(center, -direction.ux * halfLengthM, -direction.uy * halfLengthM);
  const end = moveLatLngByMeters(center, direction.ux * halfLengthM, direction.uy * halfLengthM);
...
function stationCenterFromAxis(axis: SubwayAxisShape): LatLng {
  if (axis.latLngs.length === 1) {
    const [lat, lng] = axis.latLngs[0];
    return { lat, lng };
  }

  let totalLengthM = 0;
  for (let index = 0; index < axis.latLngs.length - 1; index += 1) {
    const [startLat, startLng] = axis.latLngs[index];
    const [endLat, endLng] = axis.latLngs[index + 1];
    totalLengthM += latLngDistanceM({ lat: startLat, lng: startLng }, { lat: endLat, lng: endLng });
  }
  ...
  for (let index = 0; index < axis.latLngs.length - 1; index += 1) {
    ...
    const end = { lat: endLat, lng: endLng };
    const segmentLengthM = latLngDistanceM(start, end);
    if (cursorM + segmentLengthM >= halfLengthM) {
      const t = segmentLengthM <= 0 ? 0 : (halfLengthM - cursorM) / segmentLengthM;
      const [lat, lng] = interpolateLatLng(start, end, t);
      return { lat, lng };
    }
    cursorM += segmentLengthM;
  }

  const [lat, lng] = axis.latLngs[axis.latLngs.length - 1];
  return { lat, lng };
}

function entrancesForAxis(axis: SubwayAxisShape): SubwayEntranceShape[] {
  const first = axis.latLngs[0];
  const last = axis.latLngs[axis.latLngs.length - 1];
  const start = { lat: first[0], lng: first[1] };
  const end = { lat: last[0], lng: last[1] };
  const { ux, uy } = vectorFromLatLngs(start, end);
  const nx = -uy;
  const ny = ux;
  const offsetM = 32;
  const center = stationCenterFromAxis(axis);
  const hasLongName = subwayStationDisplayName(axis.station.name).length >= 6;
  const templates = hasLongName
    ? [
        { ref: "1", base: start, side: 1 },
        { ref: "2", base: center, side: -1 },
        { ref: "3", base: end, side: 1 },
      ]
    : [
        { ref: "1", base: start, side: 1 },
        { ref: "2", base: end, side: -1 },
      ];

  return templates.map((template) => {
    const point = moveLatLngByMeters(template.base, nx * offsetM * template.side, ny * offsetM * template.side);
    return {
      ...point,
      ref: template.ref,
      station: axis.station,
      color: axis.color,
      connectorFrom: template.base,
    };
  });
}

function subwayLineLabelPlacement(
  route: SubwayRoute,
  coordinates: readonly LatLngTuple[],
  map: import("leaflet").Map
): { lat: number; lng: number; angle: number } | null {
  const bounds = map.getBounds().pad(0.12);
  const candidates: Array<{ lat: number; lng: number; angle: number; length: number }> = [];

  for (let index = 0; index < coordinates.length - 1; index += 1) {
    const [startLat, startLng] = coordinates[index];
    const [endLat, endLng] = coordinates[index + 1];
    const midpoint: LatLngTuple = [(startLat + endLat) / 2, (startLng + endLng) / 2];
    if (!bounds.contains(midpoint)) {
      continue;
    }

    const startPoint = map.latLngToContainerPoint([startLat, startLng]);
    const endPoint = map.latLngToContainerPoint([endLat, endLng]);
    const dx = endPoint.x - startPoint.x;
    const dy = endPoint.y - startPoint.y;
    const length = Math.hypot(dx, dy);
    if (length < 36) {
    ...
    candidates.push({ lat: midpoint[0], lng: midpoint[1], angle, length });
  }
  ...
  if (route.stationIds.length === 0 || coordinates.length < 2) {
    return null;
  }

  const middle = coordinates[Math.floor(coordinates.length / 2)];
  return { lat: middle[0], lng: middle[1], angle: 0 };
}

function addOsmSubwayOverlay(
...
function addSubwayOverlay(
  L: typeof import("leaflet"),
  map: import("leaflet").Map,
  subwayLayer: import("leaflet").LayerGroup,
  stations: readonly SubwayStation[],
  allSubwayStations: readonly SubwayStation[],
  routes: readonly SubwayRoute[]
) {
  const stationById = new Map(allSubwayStations.map((station) => [station.id, station]));
  const visibleStationIds = new Set(stations.map((station) => station.id));

  routes.forEach((route) => {
    const coordinates = routeCoordinatesFor(route, stationById);
    ...
    subwayLayer.addLayer(
      L.polyline(coordinates, {
        interactive: false,
        color: subwayLineColor(route.lineColor),
        weight: 3.8,
        opacity: 0.72,
        lineCap: "round",
        lineJoin: "round",
    ...
    const placement = subwayLineLabelPlacement(route, coordinates, map);
    if (!placement) {
      return;
    }
    ...
    subwayLayer.addLayer(
      L.marker([placement.lat, placement.lng], {
        ...
        icon: L.divIcon({
          className: "site-subway-line-label-wrap",
          html: subwayLineLabelHtml(subwayLineLabelText(route.line), route.lineColor, placement.angle),
          iconSize: [0, 0],
          iconAnchor: [0, 0],
        }),
      })
  ...
  const axes = stations
    ...
    .map((station) => axisForStation(station, routes, stationById));

  axes.forEach((axis) => {
    subwayLayer.addLayer(
      L.polyline(axis.latLngs, {
        interactive: false,
        color: "#ffffff",
        weight: 13.5,
        opacity: 0.86,
  ...
  const entrances = axes.flatMap(entrancesForAxis);
  entrances.forEach((entrance) => {
    const connector: LatLngTuple[] = [
      [entrance.connectorFrom.lat, entrance.connectorFrom.lng],
  ...
  entrances.forEach((entrance) => {
    const marker = L.marker([entrance.lat, entrance.lng], {
      ...
      icon: L.divIcon({
        className: "site-subway-entrance-wrap",
        html: subwayEntranceIconHtml(entrance.ref),
        iconSize: [16, 16],
        iconAnchor: [8, 8],
      }),
    });
  ...
  axes.forEach((axis) => {
    ...
    subwayLayer.addLayer(
      L.marker([center.lat, center.lng], {
        ...
        icon: L.divIcon({
          className: "site-subway-station-wrap",
          html: subwayStationIconHtml(axis.station.name, axis.line, axis.color),
          iconSize: [0, 0],
          iconAnchor: [0, 0],
        }),
      })
...
const MapView = forwardRef<MapViewHandle, MapViewProps>(function MapView(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(OSM_SUBWAY_DATA_PATH, { cache: "force-cache" });
        if (!response.ok) {
  ...
  const updateMarkers = useCallback(async () => {
    ...
    if (routeLinesRef.current) {
      ...
      if (layers.subway) {
        if (subwayMapData) {
          addOsmSubwayOverlay(L, map, routeLinesRef.current, config, subwayMapData);
        } else {
          const visibleSubwayStations = visiblePois.filter(
            (poi): poi is SubwayStation => poi.category === "subway"
          );
          const allSubwayStations = pois.filter(
            (poi): poi is SubwayStation => poi.category === "subway"
          );
          addSubwayOverlay(L, map, routeLinesRef.current, visibleSubwayStations, allSubwayStations, subwayRoutes);
        }
      }
    }

    const clusteredPois = visiblePois.filter((poi) => poi.category !== "subway");
  ...
  }, [config, layers, pois, subwayMapData, subwayRoutes]);
